...
// here there be dragons and magic
// may look into Vercel's AI SDK for a general agent implementation for inspiration later
export class Agent {
  constructor(
  ...
  async run() {
    ...
    console.log("Chat with Claude brought to you by Indest Labs (use 'ctrl-c' to quit)")

    let readUserInput = true
    while (true) { // and we are looping
      if (readUserInput) {
        const userMessage: Anthropic.MessageParam = {
          role: "user",
          content: await this.getUserMessage(),
        }
        conversation.push(userMessage) // store conversation
      }
        ...
        for (const message of result.content) {
          switch (message.type) {
            case "text":
              ...
              break
            case "tool_use": // unbridling the beast
